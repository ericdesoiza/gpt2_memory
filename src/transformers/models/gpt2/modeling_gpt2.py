...
class GPT2BlockAugmented(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        # Eric - Added in conditional to allow for the Memory Block instead of the feed forward
        if layer_idx in config.augmented_layer_idxs:
            self.feed_forward = HashingMemory(config.hidden_size, config.hidden_size)
        else:
            self.feed_forward = GPT2MLP(inner_dim, config)
...
class GPT2ModelAugmented(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.drop = nn.Dropout(config.embd_pdrop)
        self.h = nn.ModuleList([GPT2BlockAugmented(config, layer_idx=i) for i in range(config.num_hidden_layers)]) # Eric - Swapped out GPT2Block instances
        self.ln_f = nn.LayerNorm(self.embed_dim, eps=config.layer_norm_epsilon)
...
#################### ERIC ####################
# Pulled from the GPT2LMHeadModel class, 
# added in the Augmented GPT2Model
# instead of the standard GPT2Model that
# don't support Memory
##############################################
@add_start_docstrings(
    """
    The Memory-augmented GPT2 Model transformer with a language modeling head on top (linear layer with weights tied to the input
    embeddings).
    """,
    GPT2_START_DOCSTRING,
)
class GPT2LMHeadModelAugmented(GPT2PreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.transformer = GPT2ModelAugmented(config) # Eric - Changed from original GPT2Model
        self.lm_head = nn.Linear(config.n_embd, config.vocab_size, bias=False)
